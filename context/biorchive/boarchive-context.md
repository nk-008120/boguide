...
## Open questions the next session needs to resolve with the user before building

1. **Scope of "comparable"** — full parity (multi-olympiad, multi-round, per-problem breakdown, scores/cutoffs, results) is a lot of data entry/collection work, separate from the engineering. Does the user want the full aoXiv-style depth from day one, or an MVP (e.g. just IBO + USABO, problems-and-solutions only, no scores/cutoffs/results matrices) that grows over time?
2. **Which olympiads/years/papers actually get archived first**, and where that content comes from — this is a real content-sourcing job, not just a template. The existing "Practice Papers" Google Drive folder (referenced in `content/resources/_index.md`) may be a starting source; confirm with the user what's actually available to seed this with.
...
2. Get the open questions above answered by the user **before** writing code — this is a big enough feature that guessing on data architecture will be expensive to unwind.
