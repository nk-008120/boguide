...
- Founder-stated future features (their own words, in priority order they said a survey would help rank): **(1)** a paper archive — **already built, this is BiOrchive**, **(2)** an "indigenous biology competition" — not started, **(3)** an "indigenous AI agent" to guide prep — not started, **(4)** a comments/feedback/discussion section — not started.
- A Google Forms survey ran **June 3 – July 21, 2026** (now closed as of this writing) to prioritize those features and shaped at least one real content decision already (Bioenergetics was deliberately scoped down after a "depth is unknown, people just keep reading hundreds of pages" complaint — see `handoff.md`). **Worth pulling the actual response data before any scope-expansion conversation** — it exists but this doc doesn't have it.
...
- **IBO 2022 Theoretical 1**: 49/50 questions (Q50 outstanding)
- **IBO 2022 Theoretical 2**: 50/50 — complete
- **IBO 2024 Theoretical A**: 50/50 — complete (finished this session)
...
## 6. Login + leaderboard system (built this year, in phases)
...
- Signup/login/password-reset, at `/account/` (redesigned this session as a split-screen page; old `/papers/account/` URL still redirects there).
...
- Leaderboard: a "Hall of Fame" overall aggregate (podium for top 3, ranked list below) plus one leaderboard per test round (currently 3: IBO 2022 T1, T2, IBO 2024 TA).
...
- `static/avatars/` currently has only 3 files (`avatar-01.jpg`, `avatar-02.jpg`, `avatar-03.jpg`), but the avatar-picker code expects 12 files named `avatar-01.png` … `avatar-12.png`. Right now the picker mostly shows broken/empty slots. Either more avatars need adding (matching the code's expected names) or the code needs updating to match what's actually on disk — a quick fix either way, just not done yet.
- No moderation tooling at all: no way to rename/ban/hide a bad `display_name` or leaderboard entry short of editing the Supabase table by hand. Worth having *before* the leaderboard gets real traffic, given the audience skews toward minors.
- No privacy policy page exists, and no self-serve "delete my account" flow (the data model supports it — cascading deletes — there's just no UI button for a user to trigger it).
- COPPA (US, under-13)/GDPR-minor provisions haven't been specifically reviewed, given the site now collects email, country, and education level from what's likely a lot of minors.
- Local dev and production currently point at the same live Supabase project — no separate staging database.

Full technical detail on all of this (schema, RLS design, exact file list, every bug already hit and fixed) lives in `LOGIN_ROADMAP.md` — read that before any engineering session touches this system again, don't re-derive it.
...
- **Channels**: Discord, GitHub, Instagram (`@bioguideofficial`) — all linked in the navbar, all small/early. No other channels currently exist (no TikTok/X/YouTube/Reddit/newsletter).
- **No analytics set up** — no Vercel Analytics, no Google Analytics, no tracked signup/retention numbers. Any growth conversation is starting from zero visibility into current traffic, not just zero traffic.
...
- Given zero analytics today, is step zero "add basic, privacy-respecting analytics" before any outreach push, so growth work has something to measure against?
- Given the leaderboard/profile system is genuinely public-facing now (flags, avatars, display names, no moderation), does *outreach* timing need to wait on at least minimal trust & safety work (a report mechanism, a hide/ban flag), or is that an acceptable ship-now-fix-later risk for a small early audience?
