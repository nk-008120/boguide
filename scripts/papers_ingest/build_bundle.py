...
raw.json + a transcribed answers CSV — so composing it is no longer a Claude
task at all. Paste the script's output straight into a free AI model
(Gemini/AI Studio, ChatGPT, etc.); paste its raw response into a .yaml/.txt
file for assemble.py.
...
from subjects import discover_subjects, format_subject_list_for_prompt  # noqa: E402
...
def load_answers(csv_path):
    answers = defaultdict(dict)  # number -> {letter: bool}
