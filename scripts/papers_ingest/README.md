...
for the *why* and the division of labor (script / human / free AI model /
Claude); this file is the *how* for each script. All Python, all tested
...
whoever/whatever is transcribing (you, a team member, or a free vision
model) along with the CSV schema the script prints
(`question_number,letter,answer`). This is the step that used to cost Claude
vision tokens directly — now it costs nothing on Claude's side.
...
Paste `out.txt` into a free AI model (Gemini/AI Studio, ChatGPT, etc.), save
its raw response to a file — no cleanup needed, `assemble.py` parses it
defensively.
...
## What's still a human/Claude judgment call, on purpose
