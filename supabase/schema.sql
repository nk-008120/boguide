-- BiOrchive login + leaderboard schema.
-- Run this once, top to bottom, in the Supabase project's SQL editor
-- (Dashboard -> SQL Editor -> New query). Safe to re-run: every statement
-- is idempotent (create-if-not-exists / drop-if-exists first).
--
-- See ../SETUP.md for the surrounding step-by-step (project creation,
-- env vars, etc.) this script is one step of.

create extension if not exists pgcrypto;

-- ============================================================
-- profiles — one row per auth.users row, public-readable display name.
-- No email column: email stays inside Supabase-managed auth.users only.
-- ============================================================
...
-- ============================================================
-- attempt_reports — append-only submission history (never overwritten).
-- One row per "Submit to Leaderboard" click. report_type exists so a
-- future non-quiz report kind can reuse this table without a migration.
-- ============================================================
...
-- attempt_reports: a user may read only their OWN rows (their history).
-- Deliberately NO insert/update/delete policy for anon/authenticated —
-- the only write path is the serverless function's service_role client,
-- which bypasses RLS entirely. A logged-in client can never INSERT a
-- score directly, even with a valid session.
...
-- ============================================================
-- Leaderboard views — derived, always live, never stale.
-- Views run with the owner's privileges, so they can safely aggregate
-- across all of attempt_reports while only ever exposing ranked/
-- aggregated columns, never one user's raw per_question/subject_stats
-- detail to another user.
-- ============================================================
