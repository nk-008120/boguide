...
/* Synchronous (not defer) and runs exactly here, before the rest of the
   ...
   display:none by default in CSS; this decides whether to reveal it
   BEFORE first paint, so a visitor who shouldn't see it (already opened
   this session, or prefers-reduced-motion) never sees it flash on then
   get hidden a beat later by the deferred script. */
(function () {
  var el = document.getElementById('bioclash-curtains');
  if (!el) return;
  var skip = false;
  try { skip = sessionStorage.getItem('bioclash-curtains-opened') === '1'; } catch (e) {}
  if (window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches) skip = true;
  if (!skip) el.style.display = 'block';
})();
</script>
