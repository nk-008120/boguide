...
*Note that from the reference provided by the IBO OC, it seems they are using ImageJ (older) than Fiji (newer ImageJ), hence i shall also only refer ImageJ resources here, however the difference between the two is not felt in lengths at the level we aim to familiarize ourselves with.*
...
This page explains the core features of ImageJ at the level required for the IBO plant computational biology practical. Where relevant, I have also explained the basic principles behind the tools, so you understand *why* you are using them, not just *how*.
...
- **Fiji** comes with a large collection of pre-installed plugins, better support for multi-dimensional images, and an automatic updater. It's the recommended version to use for the IBO.
...
| :--- | :--- | :--- | :--- |
| **Type** | `8-bit`, `16-bit`, `32-bit`, `RGB Color` | Converts the image to a different bit-depth. | Digital images are grids of numbers (pixels). The bit-depth determines the range of possible values (e.g., 0-255 for 8-bit). You often convert to 8-bit for simple thresholding. |
...
#### Analyze Menu (The Most Important for the IBO Practical)
This menu is for taking quantitative measurements. This is the main event for most IBO bioinformatics tasks.
...
#### Plugins Menu
You can skip this for the IBO. While many powerful tools are here, the IBO practical will likely provide you with the exact plugins you need or you will not be expected to use them. For now, just **"Explore for that"** as you indicated.
...
#### The IBO 2022/2023 Connection
In the IBO 2022/2023 practicals, you were expected to use bioinformatics tools to analyse data sets and derive biological conclusions. The same principle applies here with ImageJ. You will be given images (for example, from a chlorophyll fluorescence experiment or a root‑shoot allocation measurement) and asked to measure features. You will use thresholding to segment the plant material, `Set Measurements` to define what to measure (e.g., area, mean intensity), and then `Measure` to get your quantitative results.

### 💡 Summary: Your Workflow for the IBO Plant Practical

1. **Load** the image (`File > Open` or `File > Import` for stacks).
2. **Calibrate** the scale: If a scale bar is present, use a line selection to draw its exact length and then `Analyze > Set Scale` to convert pixels to known units (e.g., µm). *Note: The IBO may provide pre‑calibrated images.*
...
By understanding these core concepts and features, you will be well-prepared for the ImageJ section of the IBO plant computational biology practical.

---

## Plant Computational Biology – Practice Problems

<span class="badge-challenge">⚡ Practice Problems</span> This set of five problems simulates the ImageJ section of the IBO 2026 plant computational biology practical. Use them to familiarise yourself with scale calibration, segmentation, colour thresholding, fluorescence quantification, and automated particle analysis.
