...
![Five worked codon-substitution examples (A-E) classifying different outcomes: a nonsense/termination substitution (UCG→UAG, serine to stop); a missense/nonsynonymous substitution changing the encoded amino acid (UGG→UUG, tryptophan to leucine); a multi-substitution example; a synonymous (silent) substitution preserving the encoded amino acid (ACC→ACU, both threonine); and a substitution changing one amino acid while others stay fixed. Note: this illustrates the underlying synonymous/nonsynonymous/nonsense substitution classification that a dN/dS calculation is built from, rather than a full worked numeric dN/dS ratio calculation](/EVOLUTIONPICS/dn-ds-worked-example.jpg)
