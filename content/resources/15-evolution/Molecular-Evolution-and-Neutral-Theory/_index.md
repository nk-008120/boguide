...
![Five worked codon-substitution examples (A-E) classifying different outcomes: a nonsense/termination substitution (UCG→UAG, serine to stop); a missense/nonsynonymous substitution changing the encoded amino acid (UGG→UUG, tryptophan to leucine); a multi-substitution example; a synonymous (silent) substitution preserving the encoded amino acid (ACC→ACU, both threonine); and a substitution changing one amino acid while others stay fixed.](dn-ds-worked-example.jpg)
...
![Five possible fates of a duplicated gene: (a) nonfunctionalization: one paralog accumulates disabling mutations and is lost (pseudogenization); (b) neofunctionalization, one paralog acquires a new function while the other retains the ancestral one; (c) subfunctionalization: both paralogs partially diverge, jointly retaining the ancestral function between them; (d) no functional divergence (genetic robustness), both paralogs remain redundant; (e) duplication of entire gene families, which can undergo concerted evolution and gene conversion across multiple copies](gene-duplication-fates.webp)
