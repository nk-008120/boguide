...
- Genetic drift line-graph example: multiple simulated allele-frequency trajectories over generations in a small vs. large population, side by side
- Founder effect diagram: a small subset of individuals (with labeled allele frequencies) leaving a source population to found a new, isolated population
- Northern elephant seal population-size-over-time graph annotated with the historical bottleneck and current genetic diversity data point
- Gene flow diagram: two populations with different local allele frequencies, connected by migrant arrows, converging toward similar frequencies
- Summary diagram of all four mechanisms (selection, drift, gene flow, mutation) as inputs feeding into a single "allele frequency change" output, distinguishing directed (selection) from non-directed (the other three) inputs
