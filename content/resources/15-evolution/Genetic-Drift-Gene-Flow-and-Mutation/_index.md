...
![Founder effect diagram: a large source population containing two allele variants (shown as two colors) in roughly even proportion produces a small founding subpopulation with a different, chance-determined color proportion, which then diverges further as it grows into its own isolated population, a qualitative color/proportion representation rather than one with explicit numeric allele-frequency labels](founder-effect-diagram.svg)
*Source: Wikipedia*
