...
- *(built later — see project workflow)* A chromatography mode selector: given a purification goal (e.g. "isolate a His-tagged protein from a lysate," "estimate native molecular weight," "separate proteins differing mainly in surface charge"), choose the correct technique and see the separation animated.

**Static**

- SDS-PAGE gel diagram showing a protein ladder alongside sample lanes, with the size-migration relationship illustrated
- Size-exclusion chromatography schematic showing large molecules bypassing bead pores while small molecules take the longer path
- Ion-exchange chromatography schematic showing charge-based binding and salt-gradient elution
