...
![GPCR activation cycle: agonist binding triggers GDP release from Gα and GTP loading, Gα-GTP dissociates from Gβγ to activate effectors and produce a cellular response, β-arrestin binding leads to receptor internalisation into an early endosome, and the receptor is either recycled back to the membrane or degraded in the lysosome](gpcr-activation-cycle.webp)
...
![PLC cleaves membrane PIP2 into IP3 and DAG; IP3 diffuses to the ER and opens an IP3-sensitive Ca2+ channel, releasing stored Ca2+; DAG remains in the membrane and, together with the released Ca2+, activates protein kinase C, which phosphorylates downstream substrates](plc-ip3-dag-pathway.png)
