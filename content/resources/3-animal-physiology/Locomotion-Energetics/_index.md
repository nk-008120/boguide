...
![Classic Schmidt-Nielsen (1972) energy cost of locomotion chart: minimum cost of transport plotted against body mass on logarithmic axes for swimming, flying, and running animals, each as its own trend line: swimmers sit lowest, fliers intermediate, and runners highest at any given body mass, with cost of transport decreasing with increasing body mass within each mode](cost-of-transport-swim-fly-run.svg)
...
![Five-panel running-stride analysis: (a) trajectory of the body's center of mass through touchdown, stance, and take-off, modeled as a point mass on a spring-like stance leg; (b) vertical ground reaction force vs. time compared between a real human trace and the spring-mass model; (c) power vs. time, showing negative power (energy absorbed, stored in the tendon) during early stance and positive power (energy returned) during late stance; (d) a vertical work loop; (e) cost of transport vs. ground slope, human data compared to the idealized spring-mass model](tendon-elastic-energy-storage.jpg)
