...
- Feedback loop generic diagram (receptor → control center → effector) with a labeled worked example (thermoregulation) alongside a labeled positive-feedback example (blood clotting), side by side for contrast
- Nitrogenous waste trade-off diagram: ammonia/urea/uric acid structures with toxicity/cost/water-loss arrows
- Nephron diagram with countercurrent multiplier concentration gradient overlaid along the medulla (numeric mOsm values at several depths)
- Osmoconformer vs. osmoregulator, generic cell/organism diagram showing internal vs. external osmolarity under each strategy
