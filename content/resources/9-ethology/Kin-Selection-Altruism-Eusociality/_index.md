...
*Source: SlideShare — confirm licensing basis before public deployment. Presents the general rule and its algebraic forms rather than a specific numeric worked example (e.g. actual r/B/C values for the ground squirrel case) as originally requested; also introduces "spitefulness" as a related but distinct concept not covered in the text.*
...
*Source: sciencedirect.com (Current Biology) — confirm licensing basis before public deployment. The genuine original experimental-design figure behind the field data described in the text.*
...
> **Flagged — not usable:** the sourced image (`haplodiploidy-relatedness-diagram.jpg`) is a standard *diploid* kinship pedigree (parents, siblings, nieces/nephews, grandchildren, all at the ordinary r = 0.5 / 0.25 values) — it contains no haploid/diploid distinction and does not show the elevated r = 0.75 sister-sister relatedness that is the entire point of this subsection. Inserting it under this heading would visually contradict the exact claim being taught. Not inserted; flagging in case a genuine haplodiploidy-specific pedigree should be re-sourced (the "Haplodiploidy Relatedness Pedigree" interactive widget above already covers this correctly, for what it's worth).
...
*Source: fiveable.me (by way of, ultimately, a primary comparative-eusociality study) — confirm licensing basis before public deployment. A more specific research figure (tracking gut microbiota across the eusociality gradient) than a simple three-criteria diagram, but genuinely shows real Hymenoptera/termite taxa spanning the reproductive/worker caste-differentiation spectrum this section describes.*
...
*Source: ResearchGate — confirm licensing basis before public deployment. A more analytically sophisticated framing (a continuous 2D parameter space) than a simple hypothesis-vs-resolution timeline, but directly on-topic — shows precisely how kin selection and group selection relate rather than treating them as a flat binary.*
...
**Static** *(placed inline in Key Concepts above, next to the concept each one illustrates, rather than collected here. The haplodiploidy diagram was sourced but flagged as unusable — see the note under "Eusociality and Haplodiploidy" above.)*
