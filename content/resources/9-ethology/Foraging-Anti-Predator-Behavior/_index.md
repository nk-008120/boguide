...
- Oystercatcher mussel prey-choice diagram: energy payoff and handling cost curves vs. mussel size, with the profitability-maximizing intermediate size marked
- Marginal value theorem tangent-line diagram (Charnov's original graphical solution)
- Diagram contrasting the dilution effect (fixed predator capture number, more targets) against the selfish herd effect (repositioning within a fixed group)
- Periodical cicada emergence timeline showing population surge relative to estimated predator consumption capacity
- Peacock butterfly eyespot deimatic display, wings closed vs. flashed open

<!-- VIDEO: Cheetah pursuit vs. Thomson's gazelle stotting and zig-zag evasion — Acinonyx jubatus / Eudorcas thomsonii — the actual pursuit trajectory and evasive cutting are the behavioral arms race made visible, which text alone underexplains -->
