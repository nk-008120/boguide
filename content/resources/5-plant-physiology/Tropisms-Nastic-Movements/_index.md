...
![Positive thigmotropism in a stem tendril: a tendril on a weak-stemmed climbing plant grows freely until it contacts a support, then coils tightly around it, converting an otherwise unsupported stem into one that can climb upward](tendril-coiling-thigmotropism.jpg)
...
![Mimosa pudica seismonastic response: (a) unstimulated leaflets held open; (b) leaflets folded within seconds of mechanical stimulation; (c) the pulvinus (motor organ) in cross-section, showing flaccid cells on one side and turgid cells on the other, the asymmetric turgor loss producing the fold](mimosa-pulvinus-states.jpg)
...
![Venus flytrap closure sequence after trigger-hair stimulation: two independent trials (A, B) each show the trap open at 0s and progressively closing over roughly 3 seconds following mechanical stimulation of the trigger hairs](venus-flytrap-closure.webp)
