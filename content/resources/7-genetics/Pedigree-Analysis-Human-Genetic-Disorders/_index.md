...
- *(built later — see project workflow)* A pedigree pattern diagnostic tool: given a generated pedigree, click through the diagnostic checklist (father-to-son transmission present? generations skipped? sex ratio skewed?) to arrive at the correct inheritance-pattern classification.

**Static**

- Pedigree symbol key (squares/circles, filled/half-filled/unfilled, mating lines, generation numbering) as a standalone reference graphic
- Four side-by-side worked pedigree diagrams, one per classical pattern (autosomal recessive, autosomal dominant, X-linked recessive, X-linked dominant), each annotated with its diagnostic signature
- Hemophilia A pedigree diagram tracing transmission through Queen Victoria's descendants across European royal houses
- Karyotype image of a normal human chromosome complement, annotated with banding pattern and size-ordering convention
