...
- *(built later — see project workflow)* A polygenic trait distribution simulator: adjust the number of contributing gene pairs and watch the F2 phenotype distribution shift from a few discrete classes toward a smooth, continuous bell curve.

**Static**

- Polygenic trait distribution diagram showing increasing numbers of contributing gene pairs (1, 2, 4 genes) approaching a normal distribution
- Heritability variance-partitioning diagram (V_P = V_G + V_E, with V_G further split into V_A and non-additive components)
- Worked pedigree diagram calculating the inbreeding coefficient F for a first-cousin mating
- Genotype frequency comparison diagram: Hardy-Weinberg expectation (p²:2pq:q²) vs. inbred population genotype frequencies at the same allele frequencies, showing the heterozygote deficit
