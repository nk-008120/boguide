...
// Recomputes and stores a Timed Attempt result. The client sends raw
// per-statement answers, never a score — this function is the single
// place that decides what's "correct", checked against the same
// data/papers/<olympiad>/<year>.yaml Hugo already builds the site from.
// See ../supabase/schema.sql for the attempt_reports table/RLS this writes
// through (service_role bypasses RLS by design — clients can never insert
// a row directly).
const fs = require('fs');
